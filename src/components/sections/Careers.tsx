				...
				<Container>
					<div className="grid items-center justify-center gap-8 lg:grid-cols-2">
						...
						<div className="order-2 lg:order-1">
							...
							<div className="mt-7 max-w-125 space-y-4 text-sm leading-relaxed sm:text-base">
								<p>
			...
			<Container className="text-center py-16">
				...
				<div className="mt-12 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
					{perks.map((perk) => (
						<div
							key={perk.title}
							className="flex flex-col items-center rounded-xl border border-gray-100 bg-white px-4 py-4 shadow-sm"
						>
			...
			<Container className="mt-12 text-center">
				...
				<div className="mt-10 flex flex-col gap-3 text-left">
					{openings.map((job) => (
						<div
							key={job.title}
							className="grid grid-cols-1 gap-5 rounded-2xl border border-gray-200 bg-white px-6 py-5 shadow-sm sm:px-8 lg:grid-cols-[1.4fr_1fr_auto] lg:items-center lg:gap-8"
						>
							{/* Job Info */}
							<div className="flex min-w-0 items-center gap-5">
								...
								<div className="min-w-0">
									<div className="flex flex-wrap items-center gap-2">
										...
										{job.tag && (
											<span className="whitespace-nowrap rounded-full bg-green-100 px-3 py-1 text-[10px] font-medium text-green-700">
												{job.tag}
											</span>
										)}
									</div>
							...
							<div className="pl-19 lg:justify-self-end lg:pl-0">
								<button className="w-full whitespace-nowrap rounded-full border border-blue-200 px-6 py-2.5 text-xs font-semibold text-primary-foreground transition hover:bg-blue-50 sm:w-auto sm:text-sm">
									View Details
								</button>
							</div>
						</div>
				...
				<div className="mt-4 flex justify-center">
					<button className="rounded-full bg-primary-foreground px-6 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700">
						Send Your Resume
					</button>
				</div>
			</Container>
